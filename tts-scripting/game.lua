...
randNum
local creatureDeckLink =
    'https://raw.githubusercontent.com/bromberry-games/Zvery/master/tts/creature-full-grid.png?20000' .. randNum
local Game = {}

local abilityDeckLink = 'https://raw.githubusercontent.com/bromberry-games/Zvery/master/tts/adaptation-full-grid.png' ..
randNum
local backTemplate =
    'https://raw.githubusercontent.com/bromberry-games/Zvery/master/ivan-svg-templates/back-templates/card-back.png' ..
    randNum
...
local function createStatsCards()
    ...
    for i = 1, 4, 1 do
        for j = 1, 4, 1 do
            Spawner.GoPiece({ mutationDeckPos[1] + xSpacing * (0.6 + i * 0.1), 1, mutationDeckPos[3] -
            zSpacing * (0.6 + 0.1 * j) })
        end
    end
end
...
local function createSbuttonsForcurrentPlayer()
    ...
    if state.selectedAt ~= 1 then
        selectedStartButtons[1] = Spawner.SelectButton({ middlePos1[1], 1, middlePos1[3] + offset },
            "SelectCreatureAtZone" .. 1, state.otherPlayer)
    end
    if state.selectedAt ~= 2 then
        selectedStartButtons[2] = Spawner.SelectButton({ middlePos2[1], 1, middlePos2[3] + offset },
            "SelectCreatureAtZone" .. 2, state.otherPlayer)
    end
    if state.selectedAt ~= 3 then
        selectedStartButtons[3] = Spawner.SelectButton({ middlePos3[1], 1, middlePos3[3] + offset },
            "SelectCreatureAtZone" .. 3, state.otherPlayer)
    end
end
...
local function createSelectMutationButtons()
    ...
    for i, mutationZone in ipairs(mutationZones) do
        ...
        local button = Spawner.SelectButton({ mutationPos[1] - 3.0, mutationPos[2], mutationPos[3] },
            "SelectMutation" .. i, state.otherPlayer)
...
local function selectMutationAtZone(index)
    ...
    mutation.setPosition({ middlePos1[1] + xSpacing * state.mutationSelectedCount, 1, middlePos1[3] +
    zSpacing * playerDirection })
...
local function selectCreatureToFightAtZone(zone)
    ...
    elseif (state.currentPlayer == 'Blue') then
        state.selectedLevel = SelectCreatureWithAbility(Helpers.SelectFirstObjectInZone(zone),
            { middlePos1[1], 1, middlePos1[3] - 0.65 * zSpacing }, middlePos1, 1)
    end
...
local function createFightCreatureButtons()
    ...
    for i, creatureZone in ipairs(creatureZones) do
        ...
        local button = Spawner.SelectButton({ creaturePos[1] + 3.0, creaturePos[2], creaturePos[3] },
            "SelectCreatureToFight" .. i, state.otherPlayer)
